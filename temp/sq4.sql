...
FROM ((SELECT STRAIGHT_JOIN 1                                      AS `Coef`,
      ...
      LIMIT 500) UnionInvoice
         ...
         LEFT JOIN (SELECT GROUP_CONCAT(pc.code) ClassCodes, pic.id_instance IdInstance
                    FROM dvapay.paysystems_classificator pc
                             JOIN dvapay.paysystems_instance_class pic ON pc.id = pic.id_class
                    GROUP BY pic.id_instance) PsInstanceClass ON Invoice.id_instance = PsInstanceClass.IdInstance
         ...
         LEFT JOIN (SELECT InvoiceTax.invoice_id   invoice_id,
                           SUM(InvoiceTax.amount)  directTaxesAmount,
                           SUM(InvoiceTax.percent) directTaxesPercent
                    FROM billing.invoice_tax InvoiceTax
                             INNER JOIN billing.tax_jurisdiction InvoiceTaxJurisdiction
                                        ON InvoiceTaxJurisdiction.id = InvoiceTax.tax_jurisdiction_id AND
                                           InvoiceTaxJurisdiction.type = 'DT'
                    GROUP BY InvoiceTax.invoice_id) InvoiceDirectTaxes ON InvoiceDirectTaxes.invoice_id = Invoice.id
